# 🐷 Pork — cinematic AI launchpad

**Pork** turns one sentence into a complete, launch-ready token and deploys it to
...
> Pork is a third-party interface to the Pons protocol, not an official Pons product. Not financial advice.
...
## 🧠 What's Pork vs. the engine

Pork reuses a proven **launch engine** (the Pons v1/v2 adapters, AI generation, on-chain readers,
and API routes) and wraps it in a brand-new **cinematic UI** built from scratch — warm near-black
canvas, a rose→ember signature gradient, hairline glass, film grain, and a staged "reel" while the
AI cooks.

```
Frontend (Next.js, Pork UI)  →  Version selector (v1/v2)
...
| `src/app/`, `src/components/` | Pork's landing, studio, feed, and design system — **new UI** |
