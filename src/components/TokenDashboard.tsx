...
export function TokenDashboard({ address }: { address: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setError(null);
    // Pork's own launch record (KV) - used as an image fallback when the
    // on-chain logo is empty (common for v2), so the token page never shows a
    // blank logo for something launched through Pork.
    ...
    try {
      ...
      setError("Couldn't load this token. It may not be a Pork launch, or the chain is unreachable.");
    } catch {
  ...
  // Remove this launch from Pork's feed (creator-signed; on-chain token stays).
  async function removeFromFeed() {
    if (!account || !header) return;
    setRemoveState("busy");
    try {
      const signature = await signMessageAsync({
        message: `Remove ${header.token.toLowerCase()} from the Pork feed`,
      });
