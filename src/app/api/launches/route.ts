...
/** GET /api/launches?limit=48 - newest Pork launches (from KV). */
...
/** POST /api/launches - record a launch made through Pork. */
...
function removeMessage(token: string): string {
  return `Remove ${token.toLowerCase()} from the Pork feed`;
}
